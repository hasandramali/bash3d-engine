...
void Wcon_CreateConsole( void )
{
	...
	if( host.type == HOST_NORMAL )
	{
		...
		Q_strncpy( s_wcd.title, va( "Bash3D by bariscodefx %s", XASH_VERSION ), sizeof( s_wcd.title ));
		fontsize = 8;
	}
	else // dedicated console
	{
		...
		Q_strncpy( s_wcd.title, "Bash3d by bariscodefx Dedicated Server", sizeof( s_wcd.title ));
		fontsize = 14;
	}
