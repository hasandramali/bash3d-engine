...
void Con_DrawSolidConsole( float frac, qboolean fill )
{
	...
	if( host.developer )
	{
		...
		Q_snprintf( curbuild, MAX_STRING, "Bash3D by bariscodefx %i/%s build %i %s %s-%s", PROTOCOL_VERSION,
...
void Con_DrawVersion( void )
{
	...
	if( host.force_draw_version || draw_version )
		Q_snprintf( curbuild, MAX_STRING, "Bash3D by bariscodefx v%i/%s build %i %s %s-%s", PROTOCOL_VERSION,
					XASH_VERSION, Q_buildnum( ), Q_buildcommit( ), Q_buildos( ), Q_buildarch( ) );
	else Q_snprintf( curbuild, MAX_STRING, "Bash3D by bariscodefx v%i/%s build %i %s %s-%s", PROTOCOL_VERSION,
					 XASH_VERSION, Q_buildnum( ), Q_buildcommit( ), Q_buildos( ), Q_buildarch( ));
	Con_DrawStringLen( curbuild, &stringLen, &charH );
